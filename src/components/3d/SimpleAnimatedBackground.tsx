...
function DataLines() {
  ...
  // Generate lines with hero area consideration
  const lines = useMemo(() => {
    const lineData = []
    const heroCenter = new THREE.Vector3(0, 0, 0)
    
    for (let i = 0; i < 60; i++) { // Moderate line count
      const points = []
      ...
      const distance = heroCenter.distanceTo(startVec)
      const centerDistance = Math.max(0, Math.min(1, distance / 12))
      const baseAlpha = 0.15 + (centerDistance * 0.25) // Calmer in center
      
      for (let j = 0; j < 6; j++) {
        const noise = Math.sin(i * 0.15 + j * 0.6) * 1.5
        points.push([
          startX + noise,
          startY + j * 1.5 + noise * 0.4,
          startZ + noise * 0.25
        ])
      }
      ...
      lineData.push({
        ...
        color: new THREE.Color().setHSL(
          0.5 + Math.sin(i * 0.12) * 0.15, // Cyan to blue hues
          0.7,
          0.4 + centerDistance * 0.2
        ),
        alpha: 1.0, // Make more visible for testing
  ...
  useFrame(({ clock }) => {
    ...
    groupRef.current.children.forEach((line, i) => {
      if (line instanceof THREE.Line) {
        ...
        line.rotation.y = Math.sin(time) * 0.08
        line.position.y = Math.sin(time * 0.4) * 0.4
        
        // Subtle breathing effect
        const material = line.material as THREE.LineBasicMaterial
        material.opacity = lineData.alpha * (0.7 + Math.sin(time * 0.25) * 0.2)
      }
    })
  })
...
export default function SimpleAnimatedBackground() {
  ...
  // Fallback for reduced motion only (temporarily disable mobile check)
  ...
  return (
    ...
    >
      {/* Remove hero fade mask temporarily */}
