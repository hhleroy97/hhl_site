...
const DataTransferArt: React.FC<DataTransferArtProps> = ({
  className = '',
}) => {
  ...
  const animate = () => {
    ...
    dataStreamsRef.current.forEach((stream, _index) => {
    ...
    dataNodesRef.current.forEach((node, _index) => {
      ...
      if (node.material instanceof THREE.MeshBasicMaterial) {
        node.material.opacity =
          0.6 + Math.sin(time * 2.5 + userData.phase) * 0.2
      }
    })
